...
read -sp 'Enter the password for the NSX-T Manager: ' nsxtpassvar
echo -e '\n'
...
# Get the NSX-T segment id
nsxtSegmentID=`curl -X GET https://$nsxtmanvar/policy/api/v1/infra/segments -H "Content-Type: application/json" -u "admin:$nsxtpassvar" -k | jq '.results[]| .display_name,.unique_id' | grep -A1 $nsxtSegmentName | grep -v $nsxtSegmentName | sed 's/"//g'`
 
 
# Create a JSON input for the creation of the OrgNet using the values from the CSV file
sed "s/#orgNetworkName#/$orgNetworkName/g;s/#defaultGateway#/$defaultGateway/g;s/#prefixLength#/$prefixLength/g;s/#startAddress#/$startAddress/g;s/#endAddress#/$endAddress/g;s/#nsxtSegmentId#/$nsxtSegmentID/g;s/#orgVdc#/$orgVdc/g;s/#orgVdcUrn#/$orgVdcUrn/g" $jsonpathvar > $modjsonpath
